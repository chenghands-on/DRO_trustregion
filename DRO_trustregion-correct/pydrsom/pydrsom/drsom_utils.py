...
class TRS:
  ...
  @staticmethod
  def _norm(alpha, G):
    alpha = torch.reshape(alpha, (2,1))   # 1.7.1
    alpha = torch.reshape(alpha, (1,2)).squeeze()   # 1.7.1
  ...
  def _compute_root(Q, c, gamma, G):
    ...
    it = 0
    try:
      # alpha = TRS.lsolve(Q + G * _lmb_this, -c)  # pytorch 1.11.0
      alpha, LU = TRS.lsolve(torch.reshape(-c, (2,1)),Q + G * _lmb_this)  # pytorch 1.7.0
    except torch.linalg.LinAlgError as e:
      print(e)
      print(Q, G, _lmb_this, -c)
      # alpha = TRS.lsolve(Q + G * (_lmb_this + 1e-4), -c)
      alpha, LU = TRS.lsolve(torch.reshape(-c, (2,1)),Q + G * (_lmb_this+ 1e-4))  # pytorch 1.7.0
